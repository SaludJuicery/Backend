...
exports.getDailySales = function (req, res, next) {
    ...
    knex.select(knex.raw('DAYNAME(date)'), knex.raw('DATE(date)'), knex.raw('SUM(order_sum)')).from('orders').where(knex.raw('WEEKOFYEAR(date) = WEEKOFYEAR(NOW())')).groupByRaw(knex.raw('DATE(date)'))
      .then(function(rows) {
        console.log(rows);
