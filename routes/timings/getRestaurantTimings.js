...
exports.getRestaurantTimings = function (req, res, next) {
  ...
  //checking if required data is present
    if(!req.body.location || !req.body.day)
        return res.send({ "Message": "401" }); // Required data not found in post request

    //Using knex - much simpler than bookshelf.js
    knex.select('*').from('restaurant_hours').where({'location': req.body.location}).andWhere({'day': req.body.day}).orderByRaw("FIELD (day, 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday')")
      .then(function(rows) {
        console.log(rows);
